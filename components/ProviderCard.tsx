  ...
  return (
    <article className="card p-4 sm:p-5 flex flex-col sm:flex-row gap-4 sm:gap-6">
      <div className="flex sm:flex-col items-start gap-3 w-full sm:w-48">
        <div className="flex items-center justify-center size-12 sm:size-14 rounded-2xl bg-slate-900 border border-slate-800 text-2xl">
          {provider.logoEmoji}
        </div>
      ...
      <div className="flex-1 flex flex-col gap-3">
        ...
        <div className="flex flex-wrap gap-2 text-[11px]">
          {provider.tags.map((tag) => (
            <span
              key={tag}
              className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-slate-900/80 border border-slate-800 text-slate-200"
            >
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mt-2 text-xs sm:text-sm text-slate-300">
          ...
          <div className="flex flex-wrap gap-2 sm:justify-end">
            ...
            {provider.websiteUrl && (
              <a
                ...
                className="inline-flex items-center gap-1 rounded-full border border-slate-700 bg-slate-900 text-slate-200 px-3 py-2 min-h-[44px] sm:min-h-0 sm:py-1.5 text-xs sm:text-sm hover:border-brand-400/70 hover:text-brand-100 transition-colors"
              >
                Перейти на сайт 🔗
              </a>
            )}
            <Link
              ...
              className="inline-flex items-center gap-1 rounded-full bg-brand-500 hover:bg-brand-400 text-white font-medium px-3 py-2 min-h-[44px] sm:min-h-0 sm:py-1.5 text-xs sm:text-sm shadow-md shadow-brand-900/40 transition-colors"
            >
              Смотреть обзор 🔍
            </Link>
          </div>
